...
#include <SIMComAT.h>
#include "SIM8xx.Types.h"

#define HTTP_TIMEOUT 10000L
#define GPS_ACCURATE_FIX_MIN_SATELLITES 4
#define SIM8xx_UNAVAILABLE_PIN 255

class SIM8xx : public SIMComAT
{
private:
	uint8_t _resetPin;
	uint8_t _statusPin;
	 ...
	 */
	bool fireHttpRequest(const SIM8xxHttpAction action, uint16_t *statusCode, size_t *dataSize);
	/**
...
public:
	SIM8xx(uint8_t resetPin, uint8_t pwrKeyPin = SIM8xx_UNAVAILABLE_PIN, uint8_t statusPin = SIM8xx_UNAVAILABLE_PIN);
	~SIM8xx();	
	 ...
	 */
	SIM8xxChargingStatus getChargingState();
	...
	*/
	...
	/*
	   Get internal clock from the sim module.
	*/
	void getInternalClock(char *clock_output);

	/**
	 * Get current phone functionality mode.
	 */
	SIM8xxPhoneFunctionality getPhoneFunctionality();
	/**
	 * Set the phone functionality mode.
	 */
	bool setPhoneFunctionality(SIM8xxPhoneFunctionality fun);
	/**
	 * Configure slow clock, allowing the device to enter sleep mode.
	 */
	bool setSlowClock(SIM8xxSlowClock mode);
...
//	bool sendSms(const char* addr, const char* msg);
	 ...
	 */
	SIM8xxGpsStatus getGpsStatus(char * response, size_t responseSize, uint8_t minSatellitesForAccurateFix = GPS_ACCURATE_FIX_MIN_SATELLITES);
	/**
	 * Extract the specified field from the GPS parsed sequence as a uint16_t.
	 */
	bool getGpsField(const char* response, SIM8xxGpsField field, uint16_t* result);
	/**
	 * Extract the specified field from the GPS parsed sequence as a float.
	 */
	bool getGpsField(const char* response, SIM8xxGpsField field, float* result);
	/**
	 * Return a pointer to the specified field from the GPS parsed sequence.
	 */
	void getGpsField(const char* response, SIM8xxGpsField field, char** result);
	/**
