...
bool SIM8xx::setBearerSetting(ATConstStr parameter, const char* value)
{
	sendFormatAT(TO_F(AT_COMMAND_SET_BEARER_SETTING_PARAMETER), parameter, value);
	return waitResponse() == 0;
}

bool SIM8xx::getGprsPowerState(bool *state)
{
	uint8_t result;
...
SIM8xxNetworkRegistrationState SIM8xx::getNetworkRegistrationStatus()
{
	uint8_t stat;
	sendAT(TO_F(TOKEN_CGREG), TO_F(TOKEN_READ));
	
	if(waitResponse(TO_F(TOKEN_CGREG)) != 0 ||
		!parseReply(',', (uint8_t)SIM8xxRegistrationStatusResponse::Stat, &stat) ||
		waitResponse() != 0)
		return SIM8xxNetworkRegistrationState::Error;

	return (SIM8xxNetworkRegistrationState)stat;
}
